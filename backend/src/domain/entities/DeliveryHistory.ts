...
export class DeliveryHistory {
  ...
  constructor(props: DeliveryHistoryProps) {
    ...
    const orderD = new Date(props.orderDate.toISOString().split('T')[0]);
    
    this.isOnTime = actual <= promised;
    this.totalOrderedQuantity = props.totalOrderedQuantity;
    this.totalDeliveredQuantity = props.totalDeliveredQuantity;
    
    this.isInFull = this.totalDeliveredQuantity >= this.totalOrderedQuantity;
    this.isOtif = this.isOnTime && this.isInFull; // BR-012
    ...
    this.totalAcceptedQuantity = this.totalDeliveredQuantity - this.totalDefectiveQuantity;
    
    // Tính lead time theo ngày
