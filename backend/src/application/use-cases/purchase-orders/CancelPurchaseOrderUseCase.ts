...
export class CancelPurchaseOrderUseCase {
  ...
  public async execute(dto: CancelPurchaseOrderDto): Promise<PurchaseOrderResponseDto> {
    return this.uow.executeInTransaction(async () => {
      ...
      // 2. Execute Domain logic (Cancel Order)
      // This will throw error if status is not ORDERED
      order.cancelOrder(dto.cancelledBy, dto.reason);

      // 3. Rollback Inventory (On-Order -= orderedQuantity)
